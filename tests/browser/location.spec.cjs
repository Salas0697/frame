...
test('manual brief place is retained by Surprise without downloading locality data',async({page})=>{
 const requests=[];page.on('request',r=>{if(r.url().includes('/assets/locations/'))requests.push(r.url())});
 await page.goto('/frame/');await choose(page);await page.locator('.journeyLocation summary').click();await page.locator('#journeyLocation').selectOption('last');await page.locator('#journeyPlace').fill('Villavicencio · Colombia');await done(page,true);await note(page,'last');expect(requests).toHaveLength(0);
});
