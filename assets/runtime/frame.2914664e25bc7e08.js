...
function goToSlide(i){S.currentSlide=Math.max(0,Math.min(S.slides.length-1,i)); const el=$(`.slide[data-slide="${S.currentSlide}"]`); if(el)el.scrollIntoView({behavior:'smooth',inline:'start',block:'nearest'}); renderFilmstrip(); renderSheets(); saveProject()}
function bindStageScroll(){const wrap=$('#canvasWrap'); wrap.onscroll=()=>{const slideW=($('#stage .pageShell')?.getBoundingClientRect().width||framePreviewWidth())+12; const i=Math.round(wrap.scrollLeft/slideW); if(i!==S.currentSlide){S.currentSlide=Math.max(0,Math.min(S.slides.length-1,i)); renderFilmstrip(); renderSheets();}}}
