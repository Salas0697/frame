...
async function renderExportPage(i,sl,finish){
  ...
  try{
    ...
    for(const l of [...sl.layers].filter(l=>!l.hidden).sort((a,b)=>a.z-b.z)){
      if(l.type==='img'){
        ...
        try{
          ...
          try{
            ...
            x.beginPath();x.rect(-l.w*sc/2,-l.h*sc/2,l.w*sc,l.h*sc);x.clip();
      ...
      if(l.type==='deco'){x.save();x.translate((l.x+l.w/2)*sc,(l.y+l.h/2)*sc);x.rotate((l.rot||0)*Math.PI/180);x.fillStyle=l.color;if(l.kind==='circle'){x.beginPath();x.arc(0,0,l.w*sc/2,0,Math.PI*2);x.fill()}else{x.fillRect(-l.w*sc/2,-l.h*sc/2,l.w*sc,l.h*sc)}x.restore()}
    }
    x.filter='none';
...
const FrameJourney = (() => {
  ...
  q('journeyMore').onclick=()=>{if(session){session.all=!session.all;renderCards(session)}};
  ...
  function renderCards(current){
    ...
    const rows=current.all?current.ranked:current.ranked.slice(0,4);
    for(const [index,family] of rows.entries()){
      ...
      const tag=document.createElement('span');tag.className='journeyTag';tag.textContent=index===0?'Recomendado para tus fotos':'Colección '+String(index+1).padStart(2,'0');
      ...
      for(const sl of design.slides.slice(0,3)){
        ...
        for(const layer of sl.layers.filter(l=>l.type==='img')){
          const im=document.createElement('img');im.src=layer.photo.url;im.alt='';im.style.cssText=`left:${layer.x/340*100}%;top:${layer.y/425*100}%;width:${layer.w/340*100}%;height:${layer.h/425*100}%;transform:rotate(${layer.rot||0}deg);object-position:${50+(layer.offX||0)}% ${50+(layer.offY||0)}%;`;
          page.append(im);
        }
    ...
    q('journeyMore').textContent=current.all?'Ver recomendados':'Explorar los '+catalog.families.length+' templates';
  ...
  function open(){
    ...
    q('journeyCards').replaceChildren();q('journeyStatus').textContent='Preparando tus fotos…';q('journeyChoice').textContent='Preparando vistas previas…';
