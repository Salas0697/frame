...
const FrameJourney = (() => {
  ...
  function open(){
    ...
    q('journeyCards').replaceChildren();q('journeyStatus').textContent='Preparando tus fotos…';q('journeyChoice').textContent='Preparando previews…';
  ...
  function ready(current,photos){if(session!==current)return;current.photos=photos;current.ranked=rank(photos);current.selected=current.ranked[0].id;q('journeyStatus').textContent=photos.length+' fotos · Previews con tus imágenes';renderCards(current)}
