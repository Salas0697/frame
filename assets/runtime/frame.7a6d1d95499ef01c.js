...
function framePreviewWidth(){const top=($('#canvasWrap')?.getBoundingClientRect().top||0)+(window.scrollY||0);return Math.min(innerWidth-36,380,Math.max(300,innerHeight-top-234)*.8)}
...
function pushHistory(){S.history.push(clone({slides:S.slides,currentSlide:S.currentSlide,randomMode:S.randomMode,showSafe:S.showSafe,finish:S.finish,frameTemplateFamily:S.frameTemplateFamily||'',frameCaption:S.frameCaption||'',frameArtDirection:S.frameArtDirection||'',frameLastDesign:S.frameLastDesign||null,frameBackground:S.frameBackground||'auto',frameBackgroundColor:S.frameBackgroundColor||null,frameTreatment:S.frameTreatment||'gallery',heroPhotoId:S.heroPhotoId||null,frameLocation:S.frameLocation||null}));if(S.history.length>40)S.history.shift();S.future=[];updateUndo()}
function applySnap(x){Object.assign(S,x);S.frameLocation=x.frameLocation||null;S.frameBackgroundColor=x.frameBackgroundColor||null;S.selected=null}
function undo(){if(!S.history.length)return;S.future.push(clone({slides:S.slides,currentSlide:S.currentSlide,randomMode:S.randomMode,showSafe:S.showSafe,finish:S.finish,frameTemplateFamily:S.frameTemplateFamily||'',frameCaption:S.frameCaption||'',frameArtDirection:S.frameArtDirection||'',frameLastDesign:S.frameLastDesign||null,frameBackground:S.frameBackground||'auto',frameBackgroundColor:S.frameBackgroundColor||null,frameTreatment:S.frameTreatment||'gallery',heroPhotoId:S.heroPhotoId||null,frameLocation:S.frameLocation||null}));applySnap(S.history.pop());renderAll()}
function redo(){if(!S.future.length)return;S.history.push(clone({slides:S.slides,currentSlide:S.currentSlide,randomMode:S.randomMode,showSafe:S.showSafe,finish:S.finish,frameTemplateFamily:S.frameTemplateFamily||'',frameCaption:S.frameCaption||'',frameArtDirection:S.frameArtDirection||'',frameLastDesign:S.frameLastDesign||null,frameBackground:S.frameBackground||'auto',frameBackgroundColor:S.frameBackgroundColor||null,frameTreatment:S.frameTreatment||'gallery',heroPhotoId:S.heroPhotoId||null,frameLocation:S.frameLocation||null}));applySnap(S.future.pop());renderAll()}
...
function palFromPhoto(meta){if(meta.dominantColors?.length){const colors=meta.dominantColors.map(c=>c.rgb);return Array.from({length:Math.max(5,colors.length)},(_,i)=>colors[i%colors.length])}const base=meta.avg;const dark=[Math.max(0,base[0]-90),Math.max(0,base[1]-90),Math.max(0,base[2]-90)];const light=[Math.min(255,base[0]+95),Math.min(255,base[1]+95),Math.min(255,base[2]+95)];const hue=((Math.atan2(base[1]-128,base[0]-128)*180/Math.PI)+360)%360;const accent1=hslToRgb((hue+40)%360,.8,.58), accent2=hslToRgb((hue+200)%360,.75,.62);const bright=luma(base);return bright>128?[light,dark,accent1,accent2]:[dark,light,accent1,accent2]}
...
const avgRgb=[avg(rs),avg(gs),avg(bs)], mean=avg(brightness), variance=brightness.reduce((s,v)=>s+(v-mean)*(v-mean),0)/brightness.length;const centerWeight=variance + (Math.abs(img.width/img.height-1)<.4?20:0) + Math.min(img.width,img.height)/80;return {...photo,w:img.width,h:img.height,aspect:img.width/img.height,avg:avgRgb,brightness:mean,variance,grid,palette:palFromPhoto({avg:avgRgb}),score:centerWeight,sharpness,dominantColors:FramePhotoColors.extract(d)};}
...
function renderFilmstrip(){
 ...
 S.slides.forEach((sl,i)=>{
  ...
  for(const l of [...sl.layers].filter(l=>!l.hidden).sort((a,b)=>a.z-b.z)){
   ...
   if(l.type==='img'){el=FrameCrop.imageElement(l,l.w*sc,l.h*sc).box;el.classList.add('miniPhoto')}
    ...
    else {el.style.height=l.h*sc+'px';el.style.background=l.color;if(l.frameCut)el.style.clipPath=FrameCuts.css(l.frameCut);el.style.borderRadius=l.kind==='circle'?'50%':'0'}
   }
 ...
 });enableThumbDrag();document.dispatchEvent(new CustomEvent('frame:page-change',{detail:{index:S.currentSlide}}))
}
// A thumbnail/cell chooses one page atomically. Animated intermediate scroll positions
// must never replace that choice (WebKit dispatches them after the click handler).
function alignStage(){const wrap=$('#canvasWrap'),step=($('#stage .pageShell')?.getBoundingClientRect().width||framePreviewWidth())+12;wrap.scrollLeft=(S.currentSlide||0)*step}
function goToSlide(i){S.currentSlide=Math.max(0,Math.min(S.slides.length-1,i));S.selected=null;S.selectedType=null;alignStage();renderFilmstrip();renderSheets();saveProject()}
function bindStageScroll(){const wrap=$('#canvasWrap');wrap.onscroll=()=>{const step=($('#stage .pageShell')?.getBoundingClientRect().width||framePreviewWidth())+12;const i=Math.max(0,Math.min(S.slides.length-1,Math.round(wrap.scrollLeft/step)));if(i!==S.currentSlide){S.currentSlide=i;S.selected=null;S.selectedType=null;renderFilmstrip();renderSheets()}}}
...
// Export one page at a time. Explicitly release backing stores instead of
// depending on WebKit's garbage collector between pages / repeated exports.
let exportRenderQueue=Promise.resolve();
function renderSlideToFile(i, slide=clone(S.slides[i]), finish=S.finish){
  const run=exportRenderQueue.then(()=>renderExportPage(i,slide,finish));
  exportRenderQueue=run.catch(()=>{});return run;
}
async function renderExportPage(i,sl,finish){
  if(!sl)throw new Error('Missing export page');
  const W=1080,H=1350,sc=W/imgSize()[0],cv=document.createElement('canvas');
  cv.width=W;cv.height=H;
  try{
    const x=cv.getContext('2d',{willReadFrequently:true});
    if(!x)throw new Error('Export canvas unavailable');
    x.fillStyle=sl.bg;x.fillRect(0,0,W,H);x.filter=finishCanvasFilter(finish);
    for(const l of [...sl.layers].filter(l=>!l.hidden).sort((a,b)=>a.z-b.z)){
      if(l.type==='img'){
        const im=await loadImage(l.photo.url);
        try{
          if(im.decode)await im.decode();
          if(!im.naturalWidth||!im.naturalHeight)throw new Error('Photo could not be decoded: '+l.photo.name);
          x.save();
          try{
            x.translate((l.x+l.w/2)*sc,(l.y+l.h/2)*sc);x.rotate((l.rot||0)*Math.PI/180);
            FrameCuts.path(x,l.frameCut,-l.w*sc/2,-l.h*sc/2,l.w*sc,l.h*sc);x.clip();
            const g=FrameCrop.geometry(l,l.w*sc,l.h*sc);
            if(![g.x,g.y,g.w,g.h].every(Number.isFinite)||g.w<=0||g.h<=0)throw new Error('Invalid photo crop');
            // Clip in source coordinates too: never ask Safari to rasterize an
            // enormous zoomed original outside the visible photographic frame.
            const left=Math.max(0,g.x),top=Math.max(0,g.y),right=Math.min(l.w*sc,g.x+g.w),bottom=Math.min(l.h*sc,g.y+g.h);
            if(right>left&&bottom>top)x.drawImage(im,(left-g.x)/g.w*im.naturalWidth,(top-g.y)/g.h*im.naturalHeight,(right-left)/g.w*im.naturalWidth,(bottom-top)/g.h*im.naturalHeight,-l.w*sc/2+left,-l.h*sc/2+top,right-left,bottom-top);
          }finally{x.restore()}
          // Flush the draw before releasing its decoded source.
          x.getImageData(0,0,1,1);
        }finally{im.removeAttribute('src')}
        if(l.frameBorder){x.save();x.translate((l.x+l.w/2)*sc,(l.y+l.h/2)*sc);x.rotate((l.rot||0)*Math.PI/180);x.strokeStyle=l.frameBorderColor||'#202020';x.lineWidth=l.frameBorder*sc;x.strokeRect(-l.w*sc/2,-l.h*sc/2,l.w*sc,l.h*sc);x.restore()}
      }
      if(l.type==='deco'){x.save();x.translate((l.x+l.w/2)*sc,(l.y+l.h/2)*sc);x.rotate((l.rot||0)*Math.PI/180);x.fillStyle=l.color;if(l.kind==='circle'){x.beginPath();x.arc(0,0,l.w*sc/2,0,Math.PI*2);x.fill()}else if(l.frameCut){FrameCuts.path(x,l.frameCut,-l.w*sc/2,-l.h*sc/2,l.w*sc,l.h*sc);x.fill()}else{x.fillRect(-l.w*sc/2,-l.h*sc/2,l.w*sc,l.h*sc)}x.restore()}
    }
    x.filter='none';
    for(const l of sl.layers.filter(l=>l.type==='text'&&!l.hidden).sort((a,b)=>a.z-b.z)){
      x.save();x.translate(l.x*sc,l.y*sc);x.rotate((l.rot||0)*Math.PI/180);x.fillStyle=l.color;x.font=`${l.weight} ${Math.round(l.size*sc)}px ${ff(l.font)}`;x.textBaseline='top';
      if(l.frameCaption||l.frameLocation)l.text.split('\n').forEach((line,j)=>x.fillText(line,0,j*l.size*sc*.9));else wrapText(x,l.text,0,0,l.w*sc,l.size*sc*.9);x.restore();
    }
    if(finish==='film'){x.fillStyle='rgba(245,230,190,.06)';x.fillRect(0,0,W,H);addGrain(x,W,H,.06)}
    if(x.isContextLost?.())throw new Error('Export canvas lost');
    const blob=await new Promise((resolve,reject)=>cv.toBlob(b=>b?.size?resolve(b):reject(new Error('Empty PNG export')),'image/png'));
    if(x.isContextLost?.())throw new Error('Export canvas lost');
    return new File([blob],`FRAME_${String(i+1).padStart(2,'0')}.png`,{type:'image/png'});
  }finally{cv.width=0;cv.height=0}
}
function finishCanvasFilter(finish=S.finish){if(finish==='film')return 'contrast(1.02) saturate(.88) sepia(.13)'; if(finish==='soft')return 'contrast(.96) saturate(.93) brightness(1.02)'; if(finish==='mono')return 'grayscale(1) contrast(1.05)'; if(finish==='punchy')return 'contrast(1.08) saturate(1.18)'; return 'none'}
...
function downloadExport(blob,name){
  ...
  // Keep the URL alive while Safari hands the download to the OS.
  setTimeout(()=>URL.revokeObjectURL(url),60000);
}
async function exportPages(all){
  if(exportBusy||window.framePhotoImport?.active||!S.slides.length)return;
  exportBusy=true;
  const buttons=[$('#exportCurrentBtn'),$('#exportAllBtn'),$('#fastExport')].filter(Boolean),loading=$('#loading');
  buttons.forEach(b=>b.disabled=true);closeSheets();
  const pages=all?S.slides.map((sl,i)=>({i,sl:clone(sl)})):[{i:S.currentSlide,sl:clone(S.slides[S.currentSlide])}],finish=S.finish;
  try{
    loading.classList.add('on');loading.querySelector('span').textContent='Conservando tus fotos y encuadres';
    const files=[];
    for(const [n,page] of pages.entries()){
      loading.querySelector('b').textContent=`Exportando ${n+1} de ${pages.length}…`;
      await new Promise(resolve=>setTimeout(resolve,0));
      files.push(await renderSlideToFile(page.i,page.sl,finish));
    }
    loading.classList.remove('on');
    if(!(await shareFiles(files,all?'FRAME carrusel':'FRAME slide'))){
      if(all)downloadExport(await zipFiles(files),'FRAME_export.zip');else downloadExport(files[0],files[0].name);
    }
  }catch(e){if(e.name!=='AbortError'){console.warn('FRAME export failed',e);toast('No se pudo completar la exportación. Tus fotos se conservan.')}}
  finally{loading.classList.remove('on');buttons.forEach(b=>b.disabled=false);exportBusy=false}
}
$('#exportCurrentBtn').onclick=()=>exportPages(false);$('#exportAllBtn').onclick=()=>exportPages(true);
try{$('#resumeBtn').style.display=localStorage.getItem(SAVE_KEY)?'block':'none'}catch(e){$('#resumeBtn').style.display='none'}

;
window.FRAME_TEMPLATE_CATALOG={"schemaVersion":1,"status":"runtime","researchedAt":"2026-09-14","referenceCommit":"dc04e1ef7b8d86cc2bda734d7a028dce305e8ff4","coordinateSystem":{"units":"fractions_of_each_page","origin":"top_left","canvasAspect":[4,5],"slots":"unrotated_image_rectangles","rotation":"degrees_about_center; validate_bounds_after_rotation","sourceSlice":"normalized_region_of_one_shared_composite_crop; continuous variants reuse a single photo intentionally"},"sources":{"user_reference":{"type":"user_supplied_screenshot","appIdentity":"unknown","observed":"One carousel page with nine photos, white margins, fine gutters and a small footer caption"},"scrl":{"url":"https://apps.apple.com/us/app/scrl-photo-collage-maker/id1289057196","supports":"Structured and freeform collages, seamless panoramic carousels"},"unfold":{"url":"https://play.google.com/store/apps/details?hl=en_US&id=com.moonlab.unfold","supports":"Template collections including Film; customizable text and backgrounds"},"tezza":{"url":"https://apps.apple.com/us/app/tezza-aesthetic-photo-editor/id1393061654","supports":"Film, editorial, magazine, moodboard and minimal template families"},"canva":{"url":"https://www.canva.com/photo-collages/templates/","supports":"Minimalist, travel, scrapbook and instant-photo-style collage examples"},"frame_original":{"type":"original_composition","observed":"FRAME-authored geometry; no claim of popularity or copied commercial templates"}},"selection":{"weightsMeaning":"Product hypotheses based on user preference, not usage statistics; renormalize after eligibility and brief filters","weightScope":"Primary album family, not independent random style on every page","capacity":"photoCount refers to distinct photos required for this variant","coverage":"Every selected photo must appear; never silently discard or duplicate to fill a grid","caption":"Optional user supplied text only; absent text leaves whitespace","density":"Evaluate outer whitespace separately from photos per page","crop":"Reject unsafe subject crops even when a template is otherwise preferred","anotherOption":"Automatic selection excludes the three most recent families. Explicit family selection varies geometry within that family. Coverage and crop safety take precedence.","integrationBoundary":"After PhotoImportController brief and analysis; no additional file event handlers"},"families":[{"id":"museum_notes","name":"Museum Notes","priority":1,"initialWeight":16,"preferPurpose":["memory","showcase","story"],"preferVibe":["clean","natural"],"variants":[{"id":"museum_9","photoCount":9,"pageSpan":1,"slots":[{"x":0.08,"y":0.065,"w":0.274667,"h":0.265733},{"x":0.362667,"y":0.065,"w":0.274667,"h":0.265733},{"x":0.645333,"y":0.065,"w":0.274667,"h":0.265733},{"x":0.08,"y":0.337133,"w":0.274667,"h":0.265733},{"x":0.362667,"y":0.337133,"w":0.274667,"h":0.265733},{"x":0.645333,"y":0.337133,"w":0.274667,"h":0.265733},{"x":0.08,"y":0.609267,"w":0.274667,"h":0.265733},{"x":0.362667,"y":0.609267,"w":0.274667,"h":0.265733},{"x":0.645333,"y":0.609267,"w":0.274667,"h":0.265733}],"captionRegion":{"x":0.08,"y":0.9,"w":0.84,"h":0.06}},{"id":"museum_6","photoCount":6,"pageSpan":1,"slots":[{"x":0.08,"y":0.065,"w":0.416,"h":0.265733},{"x":0.504,"y":0.065,"w":0.416,"h":0.265733},{"x":0.08,"y":0.337133,"w":0.416,"h":0.265733},{"x":0.504,"y":0.337133,"w":0.416,"h":0.265733},{"x":0.08,"y":0.609267,"w":0.416,"h":0.265733},{"x":0.504,"y":0.609267,"w":0.416,"h":0.265733}],"captionRegion":{"x":0.08,"y":0.9,"w":0.84,"h":0.06}},{"id":"museum_4","photoCount":4,"pageSpan":1,"slots":[{"x":0.08,"y":0.065,"w":0.416,"h":0.4018},{"x":0.504,"y":0.065,"w":0.416,"h":0.4018},{"x":0.08,"y":0.4732,"w":0.416,"h":0.4018},{"x":0.504,"y":0.4732,"w":0.416,"h":0.4018}],"captionRegion":{"x":0.08,"y":0.9,"w":0.84,"h":0.06}}],"background":"#ffffff","sourceIds":["user_reference","scrl","canva"],"rotationDegrees":0,"covers":[{"id":"museum_notes_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.08,"y":0.06,"w":0.84,"h":0.8}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"museum_notes_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.15,"y":0.12,"w":0.7,"h":0.71}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"description":"Tu referencia: mosaicos limpios y una nota al pie."},{"id":"gallery_book","name":"Gallery Book","priority":1,"initialWeight":10,"preferPurpose":["memory","story","impact"],"preferVibe":["clean","natural"],"variants":[{"id":"gallery_center","photoCount":1,"pageSpan":1,"slots":[{"x":0.08,"y":0.08,"w":0.84,"h":0.76}],"captionRegion":{"x":0.08,"y":0.9,"w":0.84,"h":0.06}},{"id":"gallery_offset","photoCount":1,"pageSpan":1,"slots":[{"x":0.08,"y":0.08,"w":0.73,"h":0.77}],"captionRegion":{"x":0.08,"y":0.9,"w":0.84,"h":0.06}}],"background":"#ffffff","sourceIds":["unfold","canva"],"covers":[{"id":"gallery_book_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.12,"y":0.1,"w":0.76,"h":0.74}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"gallery_book_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.07,"y":0.08,"w":0.7,"h":0.78}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"description":"El ritmo pausado de un libro de fotografía."},{"id":"editorial_pair","name":"Editorial Pair","priority":1,"initialWeight":10,"preferPurpose":["story","memory"],"preferVibe":["clean","natural"],"variants":[{"id":"pair_vertical","photoCount":2,"pageSpan":1,"slots":[{"x":0.08,"y":0.12,"w":0.416,"h":0.7},{"x":0.504,"y":0.12,"w":0.416,"h":0.7}]},{"id":"pair_horizontal","photoCount":2,"pageSpan":1,"slots":[{"x":0.08,"y":0.08,"w":0.84,"h":0.3968},{"x":0.08,"y":0.4832,"w":0.84,"h":0.3968}]},{"id":"triptych","photoCount":3,"pageSpan":1,"slots":[{"x":0.08,"y":0.2,"w":0.274667,"h":0.55},{"x":0.362667,"y":0.2,"w":0.274667,"h":0.55},{"x":0.645333,"y":0.2,"w":0.274667,"h":0.55}]}],"background":"#ffffff","sourceIds":["scrl","tezza"],"covers":[{"id":"editorial_pair_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.05,"y":0.09,"w":0.66,"h":0.73}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"editorial_pair_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.22,"y":0.14,"w":0.73,"h":0.72}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"description":"Dípticos y trípticos para dialogar entre imágenes."},{"id":"film_archive","name":"Film Archive","priority":2,"initialWeight":8,"preferPurpose":["memory","story"],"preferVibe":["natural","bold"],"variants":[{"id":"film_strip_3","photoCount":3,"pageSpan":1,"slots":[{"x":0.17,"y":0.06,"w":0.66,"h":0.276},{"x":0.17,"y":0.352,"w":0.66,"h":0.276},{"x":0.17,"y":0.644,"w":0.66,"h":0.276}]},{"id":"film_contact_6","photoCount":6,"pageSpan":1,"slots":[{"x":0.06,"y":0.06,"w":0.43,"h":0.276},{"x":0.51,"y":0.06,"w":0.43,"h":0.276},{"x":0.06,"y":0.352,"w":0.43,"h":0.276},{"x":0.51,"y":0.352,"w":0.43,"h":0.276},{"x":0.06,"y":0.644,"w":0.43,"h":0.276},{"x":0.51,"y":0.644,"w":0.43,"h":0.276}]}],"background":"#161616","textureDefault":"none","sourceIds":["unfold","tezza"],"covers":[{"id":"film_archive_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.17,"y":0.06,"w":0.66,"h":0.8}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"film_archive_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.1,"y":0.19,"w":0.8,"h":0.63}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"description":"Tiras y hojas de contacto con ritmo de película."},{"id":"travel_journal","name":"Travel Journal","priority":2,"initialWeight":12,"preferPurpose":["story","memory"],"preferVibe":["natural","clean"],"variants":[{"id":"journal_3","photoCount":3,"pageSpan":1,"slots":[{"x":0.08,"y":0.065,"w":0.84,"h":0.49},{"x":0.08,"y":0.57,"w":0.414,"h":0.28},{"x":0.506,"y":0.57,"w":0.414,"h":0.28}],"captionRegion":{"x":0.08,"y":0.9,"w":0.84,"h":0.06}},{"id":"journal_4","photoCount":4,"pageSpan":1,"slots":[{"x":0.08,"y":0.065,"w":0.84,"h":0.49},{"x":0.08,"y":0.57,"w":0.274667,"h":0.28},{"x":0.362667,"y":0.57,"w":0.274667,"h":0.28},{"x":0.645333,"y":0.57,"w":0.274667,"h":0.28}],"captionRegion":{"x":0.08,"y":0.9,"w":0.84,"h":0.06}}],"background":"#faf8f3","sourceIds":["canva","tezza"],"covers":[{"id":"travel_journal_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.07,"y":0.06,"w":0.86,"h":0.65}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"travel_journal_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.2,"y":0.11,"w":0.72,"h":0.74}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"description":"Una imagen principal y pequeños recuerdos."},{"id":"continuous","name":"Continuous","priority":2,"initialWeight":6,"preferPurpose":["story","impact"],"preferVibe":["natural","bold"],"variants":[{"id":"continuous_2","photoCount":1,"pageSpan":2,"slots":[{"x":0,"y":0,"w":1,"h":1,"pageIndex":0,"sourceSlice":{"x":0,"y":0,"w":0.5,"h":1}},{"x":0,"y":0,"w":1,"h":1,"pageIndex":1,"sourceSlice":{"x":0.5,"y":0,"w":0.5,"h":1}}]},{"id":"continuous_3","photoCount":1,"pageSpan":3,"slots":[{"x":0,"y":0,"w":1,"h":1,"pageIndex":0,"sourceSlice":{"x":0,"y":0,"w":0.3333333333333333,"h":1}},{"x":0,"y":0,"w":1,"h":1,"pageIndex":1,"sourceSlice":{"x":0.3333333333333333,"y":0,"w":0.3333333333333333,"h":1}},{"x":0,"y":0,"w":1,"h":1,"pageIndex":2,"sourceSlice":{"x":0.6666666666666666,"y":0,"w":0.3333333333333333,"h":1}}]}],"background":"photo","sourceIds":["scrl"],"requirements":["Wide source image adequate for the combined canvas","Faces must avoid each individual page seam"],"covers":[{"id":"continuous_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0,"y":0.2,"w":1,"h":0.6}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"continuous_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.06,"y":0.04,"w":0.88,"h":0.82}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"description":"Paisajes continuos entre páginas, cuando el recorte lo permite."},{"id":"soft_scrapbook","name":"Soft Scrapbook","priority":3,"initialWeight":3,"preferPurpose":["memory"],"preferVibe":["bold"],"variants":[{"id":"scrapbook_3","photoCount":3,"pageSpan":1,"slots":[{"x":0.1,"y":0.08,"w":0.64,"h":0.38,"rotation":-2},{"x":0.4,"y":0.38,"w":0.49,"h":0.3,"rotation":2},{"x":0.1,"y":0.6,"w":0.57,"h":0.31,"rotation":-1}]},{"id":"scrapbook_4","photoCount":4,"pageSpan":1,"slots":[{"x":0.1,"y":0.08,"w":0.54,"h":0.34,"rotation":-2},{"x":0.48,"y":0.25,"w":0.42,"h":0.29,"rotation":2},{"x":0.1,"y":0.48,"w":0.46,"h":0.31,"rotation":-1},{"x":0.48,"y":0.65,"w":0.4,"h":0.26,"rotation":2}]}],"background":"#faf8f3","allowOverlap":true,"textureDefault":"none","sourceIds":["canva","tezza"],"requirements":["Check rotated bounds","Reject occluded faces and key subjects"],"covers":[{"id":"soft_scrapbook_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.15,"y":0.12,"w":0.68,"h":0.7}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"soft_scrapbook_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.1,"y":0.09,"w":0.7,"h":0.74}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"description":"Recuerdos superpuestos con giros contenidos."},{"id":"color_editorial","name":"Color Editorial","priority":3,"initialWeight":3,"preferPurpose":["impact"],"preferVibe":["color","bold"],"variants":[{"id":"color_hero","photoCount":1,"pageSpan":1,"slots":[{"x":0.08,"y":0.065,"w":0.84,"h":0.81}]},{"id":"color_pair","photoCount":2,"pageSpan":1,"slots":[{"x":0.08,"y":0.12,"w":0.416,"h":0.7},{"x":0.504,"y":0.12,"w":0.416,"h":0.7}]}],"background":"single_photo_derived_accent","sourceIds":["tezza","canva"],"covers":[{"id":"color_editorial_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.12,"y":0.12,"w":0.76,"h":0.68}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"color_editorial_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.08,"y":0.2,"w":0.7,"h":0.64}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"description":"Color tomado de tus fotos y composición sobria."},{"id":"full_bleed","name":"Full Bleed","description":"Fotos monumentales, cortes limpios y páginas hasta el borde.","priority":1,"initialWeight":12,"preferPurpose":["impact","showcase"],"preferVibe":["clean","bold"],"background":"#ffffff","sourceIds":["frame_original"],"variants":[{"id":"bleed_single","photoCount":1,"pageSpan":1,"slots":[{"x":0,"y":0,"w":1,"h":1}]},{"id":"bleed_pair","photoCount":2,"pageSpan":1,"slots":[{"x":0,"y":0,"w":0.49,"h":1},{"x":0.51,"y":0,"w":0.49,"h":1}]},{"id":"bleed_3","photoCount":3,"pageSpan":1,"slots":[{"x":0,"y":0,"w":1,"h":0.62},{"x":0,"y":0.636,"w":0.49,"h":0.364},{"x":0.51,"y":0.636,"w":0.49,"h":0.364}]},{"id":"bleed_4","photoCount":4,"pageSpan":1,"slots":[{"x":0,"y":0,"w":0.494,"h":0.494},{"x":0.506,"y":0,"w":0.494,"h":0.494},{"x":0,"y":0.506,"w":0.494,"h":0.494},{"x":0.506,"y":0.506,"w":0.494,"h":0.494}]}],"covers":[{"id":"full_bleed_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0,"y":0,"w":1,"h":1}]},{"id":"full_bleed_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0,"y":0.12,"w":1,"h":0.88}]}]},{"id":"offset_studies","name":"Offset Studies","description":"Asimetría precisa, escalas distintas y espacios inesperados.","priority":1,"initialWeight":12,"preferPurpose":["story","memory"],"preferVibe":["clean","natural"],"background":"#ffffff","sourceIds":["frame_original"],"variants":[{"id":"offset_step_2","photoCount":2,"pageSpan":1,"slots":[{"x":0.06,"y":0.07,"w":0.55,"h":0.55},{"x":0.65,"y":0.48,"w":0.29,"h":0.39}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"offset_3","photoCount":3,"pageSpan":1,"slots":[{"x":0.06,"y":0.1,"w":0.52,"h":0.75},{"x":0.64,"y":0.06,"w":0.3,"h":0.34},{"x":0.64,"y":0.46,"w":0.3,"h":0.34}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"offset_4","photoCount":4,"pageSpan":1,"slots":[{"x":0.06,"y":0.06,"w":0.38,"h":0.32},{"x":0.5,"y":0.16,"w":0.44,"h":0.47},{"x":0.06,"y":0.44,"w":0.38,"h":0.4},{"x":0.5,"y":0.69,"w":0.44,"h":0.18}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"offset_single","photoCount":1,"pageSpan":1,"slots":[{"x":0.08,"y":0.23,"w":0.68,"h":0.56}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"covers":[{"id":"offset_studies_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.04,"y":0.05,"w":0.72,"h":0.78}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"offset_studies_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.25,"y":0.26,"w":0.7,"h":0.6}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}]},{"id":"cinema_club","name":"Cinema Club","description":"Bandas panorámicas y pausas amplias, como fotogramas.","priority":1,"initialWeight":12,"preferPurpose":["story","impact"],"preferVibe":["clean","bold","natural"],"background":"#101012","sourceIds":["frame_original"],"variants":[{"id":"cinema_still","photoCount":1,"pageSpan":1,"slots":[{"x":0.035,"y":0.26,"w":0.93,"h":0.46}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"cinema_double","photoCount":2,"pageSpan":1,"slots":[{"x":0.035,"y":0.09,"w":0.93,"h":0.34},{"x":0.035,"y":0.52,"w":0.93,"h":0.34}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"cinema_triple","photoCount":3,"pageSpan":1,"slots":[{"x":0.035,"y":0.035,"w":0.93,"h":0.27},{"x":0.035,"y":0.365,"w":0.93,"h":0.27},{"x":0.035,"y":0.695,"w":0.93,"h":0.27}]},{"id":"cinema_reel","photoCount":3,"pageSpan":1,"slots":[{"x":0.04,"y":0.12,"w":0.6,"h":0.72},{"x":0.69,"y":0.12,"w":0.27,"h":0.32},{"x":0.69,"y":0.52,"w":0.27,"h":0.32}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"covers":[{"id":"cinema_club_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0,"y":0.22,"w":1,"h":0.56}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"cinema_club_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.1,"y":0.09,"w":0.8,"h":0.77}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}]},{"id":"collector","name":"Collector","description":"Fotografías pequeñas, silencio visual y márgenes generosos.","priority":1,"initialWeight":12,"preferPurpose":["memory","story"],"preferVibe":["clean","natural"],"background":"#ffffff","sourceIds":["frame_original"],"variants":[{"id":"collector_single","photoCount":1,"pageSpan":1,"slots":[{"x":0.23,"y":0.22,"w":0.54,"h":0.49}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"collector_duo","photoCount":2,"pageSpan":1,"slots":[{"x":0.1,"y":0.14,"w":0.45,"h":0.48},{"x":0.63,"y":0.52,"w":0.24,"h":0.26}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"collector_triplet","photoCount":3,"pageSpan":1,"slots":[{"x":0.14,"y":0.13,"w":0.32,"h":0.32},{"x":0.55,"y":0.27,"w":0.3,"h":0.28},{"x":0.25,"y":0.63,"w":0.35,"h":0.23}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"collector_four","photoCount":4,"pageSpan":1,"slots":[{"x":0.19,"y":0.15,"w":0.2675,"h":0.2625},{"x":0.5425,"y":0.15,"w":0.2675,"h":0.2625},{"x":0.19,"y":0.49749999999999994,"w":0.2675,"h":0.2625},{"x":0.5425,"y":0.49749999999999994,"w":0.2675,"h":0.2625}]}],"covers":[{"id":"collector_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.24,"y":0.23,"w":0.52,"h":0.48}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"collector_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.16,"y":0.15,"w":0.6,"h":0.57}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}]},{"id":"column_house","name":"Column House","description":"Una foto protagonista junto a columnas de detalles.","priority":1,"initialWeight":12,"preferPurpose":["story","memory"],"preferVibe":["clean","natural"],"background":"#f7f6f2","sourceIds":["frame_original"],"variants":[{"id":"column_3","photoCount":3,"pageSpan":1,"slots":[{"x":0.045,"y":0.05,"w":0.56,"h":0.8},{"x":0.635,"y":0.05,"w":0.32,"h":0.385},{"x":0.635,"y":0.465,"w":0.32,"h":0.385}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"column_4","photoCount":4,"pageSpan":1,"slots":[{"x":0.045,"y":0.05,"w":0.56,"h":0.8},{"x":0.635,"y":0.05,"w":0.32,"h":0.25},{"x":0.635,"y":0.325,"w":0.32,"h":0.25},{"x":0.635,"y":0.6,"w":0.32,"h":0.25}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"column_5","photoCount":5,"pageSpan":1,"slots":[{"x":0.04,"y":0.05,"w":0.56,"h":0.81},{"x":0.635,"y":0.05,"w":0.1535,"h":0.396},{"x":0.8065,"y":0.05,"w":0.1535,"h":0.396},{"x":0.635,"y":0.464,"w":0.1535,"h":0.396},{"x":0.8065,"y":0.464,"w":0.1535,"h":0.396}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"column_duo","photoCount":2,"pageSpan":1,"slots":[{"x":0.045,"y":0.05,"w":0.33,"h":0.8},{"x":0.415,"y":0.24,"w":0.54,"h":0.61}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"covers":[{"id":"column_house_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.045,"y":0.05,"w":0.64,"h":0.81}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"column_house_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.32,"y":0.05,"w":0.635,"h":0.81}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}]},{"id":"contact_press","name":"Contact Press","description":"Hojas de contacto densas, retícula fina y ritmo de archivo.","priority":1,"initialWeight":12,"preferPurpose":["showcase","memory"],"preferVibe":["clean","natural","bold"],"background":"#191919","sourceIds":["frame_original"],"variants":[{"id":"contact_12","photoCount":12,"pageSpan":1,"slots":[{"x":0.05,"y":0.045,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.35733333333333334,"y":0.045,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.6646666666666667,"y":0.045,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.05,"y":0.25675,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.35733333333333334,"y":0.25675,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.6646666666666667,"y":0.25675,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.05,"y":0.46849999999999997,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.35733333333333334,"y":0.46849999999999997,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.6646666666666667,"y":0.46849999999999997,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.05,"y":0.68025,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.35733333333333334,"y":0.68025,"w":0.2853333333333333,"h":0.18974999999999997},{"x":0.6646666666666667,"y":0.68025,"w":0.2853333333333333,"h":0.18974999999999997}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"contact_8","photoCount":8,"pageSpan":1,"slots":[{"x":0.06,"y":0.045,"w":0.428,"h":0.18824999999999997},{"x":0.512,"y":0.045,"w":0.428,"h":0.18824999999999997},{"x":0.06,"y":0.25725,"w":0.428,"h":0.18824999999999997},{"x":0.512,"y":0.25725,"w":0.428,"h":0.18824999999999997},{"x":0.06,"y":0.4695,"w":0.428,"h":0.18824999999999997},{"x":0.512,"y":0.4695,"w":0.428,"h":0.18824999999999997},{"x":0.06,"y":0.68175,"w":0.428,"h":0.18824999999999997},{"x":0.512,"y":0.68175,"w":0.428,"h":0.18824999999999997}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"contact_6_wide","photoCount":6,"pageSpan":1,"slots":[{"x":0.05,"y":0.16,"w":0.2853333333333333,"h":0.324},{"x":0.35733333333333334,"y":0.16,"w":0.2853333333333333,"h":0.324},{"x":0.6646666666666667,"y":0.16,"w":0.2853333333333333,"h":0.324},{"x":0.05,"y":0.506,"w":0.2853333333333333,"h":0.324},{"x":0.35733333333333334,"y":0.506,"w":0.2853333333333333,"h":0.324},{"x":0.6646666666666667,"y":0.506,"w":0.2853333333333333,"h":0.324}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"contact_4","photoCount":4,"pageSpan":1,"slots":[{"x":0.08,"y":0.08,"w":0.40599999999999997,"h":0.361},{"x":0.514,"y":0.08,"w":0.40599999999999997,"h":0.361},{"x":0.08,"y":0.46900000000000003,"w":0.40599999999999997,"h":0.361},{"x":0.514,"y":0.46900000000000003,"w":0.40599999999999997,"h":0.361}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}],"covers":[{"id":"contact_press_cover_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.1,"y":0.08,"w":0.8,"h":0.77}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}},{"id":"contact_press_cover_2","photoCount":1,"pageSpan":1,"slots":[{"x":0.16,"y":0.2,"w":0.68,"h":0.62}],"captionRegion":{"x":0.08,"y":0.91,"w":0.84,"h":0.06}}]},{"id":"torn_atelier","name":"Rasgado Atelier","description":"Papel rasgado, fotos amplias y dípticos con aire.","priority":2,"initialWeight":7,"preferPurpose":["story","memory"],"preferVibe":["natural","bold"],"series":"Rasgadas y cortes","cutStyle":"torn","variants":[{"id":"torn_atelier_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.1,"y":0.08,"w":0.8,"h":0.77,"rotation":-1}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"torn_atelier_2","photoCount":2,"pageSpan":1,"slots":[{"x":0.065,"y":0.09,"w":0.4,"h":0.64,"rotation":-1.5},{"x":0.535,"y":0.25,"w":0.4,"h":0.61,"rotation":1}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"torn_atelier_3","photoCount":2,"pageSpan":1,"slots":[{"x":0.09,"y":0.08,"w":0.82,"h":0.43,"rotation":0},{"x":0.18,"y":0.56,"w":0.64,"h":0.31,"rotation":-1}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}}],"covers":[{"id":"torn_atelier_cover_a","photoCount":1,"pageSpan":1,"slots":[{"x":0.09,"y":0.065,"w":0.82,"h":0.79,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"torn_atelier_cover_b","photoCount":1,"pageSpan":1,"slots":[{"x":0.16,"y":0.16,"w":0.7,"h":0.67,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}}]},{"id":"torn_horizon","name":"Horizonte Rasgado","description":"Franjas fotográficas separadas por bordes de papel.","priority":2,"initialWeight":7,"preferPurpose":["story","memory"],"preferVibe":["natural","bold"],"series":"Rasgadas y cortes","cutStyle":"horizon","variants":[{"id":"torn_horizon_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.065,"y":0.15,"w":0.87,"h":0.66,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"torn_horizon_2","photoCount":2,"pageSpan":1,"slots":[{"x":0.06,"y":0.07,"w":0.88,"h":0.37,"rotation":0},{"x":0.06,"y":0.49,"w":0.88,"h":0.37,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"torn_horizon_3","photoCount":3,"pageSpan":1,"slots":[{"x":0.06,"y":0.055,"w":0.88,"h":0.245,"rotation":0},{"x":0.06,"y":0.335,"w":0.88,"h":0.245,"rotation":0},{"x":0.06,"y":0.615,"w":0.88,"h":0.245,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}}],"covers":[{"id":"torn_horizon_cover_a","photoCount":1,"pageSpan":1,"slots":[{"x":0.09,"y":0.065,"w":0.82,"h":0.79,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"torn_horizon_cover_b","photoCount":1,"pageSpan":1,"slots":[{"x":0.16,"y":0.16,"w":0.7,"h":0.67,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}}]},{"id":"cut_diagonal","name":"Corte Diagonal","description":"Esquinas cortadas y composiciones asimétricas.","priority":2,"initialWeight":7,"preferPurpose":["story","memory"],"preferVibe":["natural","bold"],"series":"Rasgadas y cortes","cutStyle":"diagonal","variants":[{"id":"cut_diagonal_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.08,"y":0.07,"w":0.77,"h":0.77,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"cut_diagonal_2","photoCount":2,"pageSpan":1,"slots":[{"x":0.065,"y":0.07,"w":0.56,"h":0.49,"rotation":0},{"x":0.45,"y":0.6,"w":0.485,"h":0.28,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"cut_diagonal_3","photoCount":2,"pageSpan":1,"slots":[{"x":0.07,"y":0.09,"w":0.42,"h":0.7,"rotation":0},{"x":0.55,"y":0.19,"w":0.38,"h":0.61,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}}],"covers":[{"id":"cut_diagonal_cover_a","photoCount":1,"pageSpan":1,"slots":[{"x":0.09,"y":0.065,"w":0.82,"h":0.79,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"cut_diagonal_cover_b","photoCount":1,"pageSpan":1,"slots":[{"x":0.16,"y":0.16,"w":0.7,"h":0.67,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}}]},{"id":"cut_mosaic","name":"Mosaico Cortado","description":"Recortes geométricos con detalles de revista.","priority":2,"initialWeight":7,"preferPurpose":["story","memory"],"preferVibe":["natural","bold"],"series":"Rasgadas y cortes","cutStyle":"notch","variants":[{"id":"cut_mosaic_1","photoCount":1,"pageSpan":1,"slots":[{"x":0.13,"y":0.07,"w":0.74,"h":0.78,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"cut_mosaic_2","photoCount":3,"pageSpan":1,"slots":[{"x":0.07,"y":0.065,"w":0.53,"h":0.48,"rotation":0},{"x":0.66,"y":0.065,"w":0.27,"h":0.48,"rotation":0},{"x":0.07,"y":0.6,"w":0.86,"h":0.28,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"cut_mosaic_3","photoCount":4,"pageSpan":1,"slots":[{"x":0.065,"y":0.07,"w":0.4,"h":0.37,"rotation":0},{"x":0.535,"y":0.07,"w":0.4,"h":0.37,"rotation":0},{"x":0.065,"y":0.51,"w":0.4,"h":0.37,"rotation":0},{"x":0.535,"y":0.51,"w":0.4,"h":0.37,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}}],"covers":[{"id":"cut_mosaic_cover_a","photoCount":1,"pageSpan":1,"slots":[{"x":0.09,"y":0.065,"w":0.82,"h":0.79,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}},{"id":"cut_mosaic_cover_b","photoCount":1,"pageSpan":1,"slots":[{"x":0.16,"y":0.16,"w":0.7,"h":0.67,"rotation":0}],"captionRegion":{"x":0.07,"y":0.92,"w":0.86,"h":0.055}}]}],"version":"editorial-1","edition":"Editorial Library 02"};window.FRAME_LOCATION_DATA_FILE="cities.0be9ecf4754b25cc.json";
;
...
.app{padding-top:0}.nav{height:calc(58px + env(safe-area-inset-top));padding-top:env(safe-area-inset-top)}.nav .persistentExport{min-height:44px;background:#efeee8;color:#111;border-color:#efeee8;font-weight:700;padding:0 14px}.nav .persistentExport:disabled{opacity:.45}
...
function selectOnly(el){disarm();const si=+el.dataset.slide,id=el.dataset.id,type=el.classList.contains('textLayer')?'text':'img';S.currentSlide=si;alignStage();S.selected=id;S.selectedType=type;$('#uxEdit').textContent=type==='img'?'Reencuadrar':'Editar';$('#uxMove').hidden=type==='img';renderFilmstrip?.();renderSheets?.();bar.classList.add('on')}
